...
class Opportunity:
    ...
    def onDestInstBtnClick(self, *args):
        ...
        self._disk = os.popen("fdisk -l | grep Disk | head -n1 | awk '{print $2}' | cut -d ':' -f1").readline()
        self._disk = self._disk[:len(self._disk)-1]
    ...
    def onConfirmNextBtnClick(self, *args):
        ...
        if self._mode == "destructive":
            ...
            os.environ['ROOTPART'] = self._mount_data['root'][:self._mount_data['root'].find(' ')]
            if self._mount_data['efi'] != 'default':
                os.environ['EFIPART'] = self._mount_data['efi'][:self._mount_data['efi'].find(' ')]
...
if __name__ == '__main__':
    app = Opportunity('/usr/share/opportunity/ui.glade')
    app.load()
    app.run()